...
import Gallery from './components/User/Admin/Gallery'

  const Routes = () => {
    return (
      <Layout> 
        <Switch>
          <Route path="/vartotojas/informacija" exact component={Auth(Userdashboard,true)}/>
          <Route path="/admin/paslaugos" exact component={Auth(AddProduct,true)}/>
          <Route path="/admin/galerijos" exact component={Auth(Gallery,true)}/>

          <Route path="/registruotis" exact component={Auth(Register,false)}/>
          <Route path="/prisijungti" exact component={Auth(RegisterLogin,false)}/>
